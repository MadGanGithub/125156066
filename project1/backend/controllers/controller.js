...
async function getProducts(company, category,token) {
    try {
        ...
        console.error(`Error fetching data for company: ${company}, category: ${category}`, error.message);
        return [];
    }
}
...
const fetchAndStoreData= async(req,res)=> {
    ...
    await axios.post(`http://20.244.56.144/test/auth`,queryParams)
    .then(response => {
...
const getProductsByCategoryCompanyAndPrice =(data, categoryName, minPrice, maxPrice)=> {
    const flattenedData = data.flat();
    return flattenedData.filter(product =>
  ...
  const getEachProduct=(req, res) => {
    ...
    if (minPrice === undefined || maxPrice === undefined) {
      return res.status(300).json({ message: 'Category, company, minPrice, and maxPrice are required' });
    }
    console.log("inside");
    const minPriceNum = parseFloat(minPrice);
    const maxPriceNum = parseFloat(maxPrice);
  
    const result = getProductsByCategoryCompanyAndPrice(Products, categoryName, minPriceNum, maxPriceNum);
    res.json(result);
}catch(error){
    console.log(error)
}
...
const getSpecificProducts= async(req,res)=>{
    try{
        ...
        const product = Products.find(
          each => each.id === product && each.category === categoryname
        );
        console.log(product)
...
export { check, getEachProduct, getSpecificProducts };
